...
export class PoosheshComponentsPage {
    ...
    deleteButtons = element.all(by.css('jhi-pooshesh div table .btn-danger'));
    title = element.all(by.css('jhi-pooshesh div h2#page-heading span')).first();
...
export class PoosheshUpdatePage {
    pageTitle = element(by.id('jhi-pooshesh-heading'));
...
export class PoosheshDeleteDialog {
    private dialogTitle = element(by.id('jhi-delete-pooshesh-heading'));
    private confirmButton = element(by.id('jhi-confirm-delete-pooshesh'));
