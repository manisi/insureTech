...
export class MohasebeBadaneComponentsPage {
    ...
    deleteButtons = element.all(by.css('jhi-mohasebe-badane div table .btn-danger'));
    title = element.all(by.css('jhi-mohasebe-badane div h2#page-heading span')).first();
...
export class MohasebeBadaneUpdatePage {
    pageTitle = element(by.id('jhi-mohasebe-badane-heading'));
...
export class MohasebeBadaneDeleteDialog {
    private dialogTitle = element(by.id('jhi-delete-mohasebeBadane-heading'));
    private confirmButton = element(by.id('jhi-confirm-delete-mohasebeBadane'));
